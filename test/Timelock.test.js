...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.mvs.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.mvs.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.mvs.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.mvs.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.mvs.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.mvs.address, dev, '1000', '0', '1000', "2000", { from: alice });
        await this.mvs.transferOwnership(this.chef.address, { from: alice });
        await this.chef.add('100', 0, "1000", this.lp1.address, true);
        await this.chef.transferOwnership(this.timelock.address, { from: alice });
        const eta = (await time.latest()).add(time.duration.days(4));
        await this.timelock.queueTransaction(
            this.chef.address, '0', 'set(uint256,uint256,uint256,bool)',
            encodeParameters(['uint256', 'uint256', 'uint256', 'bool'], ['0', '200', "2000", false]), eta, { from: bob },
        );
        await this.timelock.queueTransaction(
            this.chef.address, '0', 'add(uint256,uint256,uint256,address,bool)',
            encodeParameters(['uint256', 'uint256', 'uint256', 'address', 'bool'], ['100', "0", "1000", this.lp2.address, false]), eta, { from: bob },
        );
        await time.increase(time.duration.days(4));
        await this.timelock.executeTransaction(
            this.chef.address, '0', 'set(uint256,uint256,uint256,bool)',
