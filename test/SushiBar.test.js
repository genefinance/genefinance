...
const MvsBar = artifacts.require('MvsBar');

contract('MvsToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.mvs = await MvsToken.new({ from: alice });
        this.bar = await MvsBar.new(this.mvs.address, { from: alice });
        this.mvs.mint(alice, '100', { from: alice });
        this.mvs.mint(bob, '100', { from: alice });
        this.mvs.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.mvs.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.mvs.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.mvs.approve(this.bar.address, '100', { from: alice });
        await this.mvs.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.mvs.balanceOf(this.bar.address)).valueOf(), '30');
        // MvsBar get 20 more MVSs from an external source.
        await this.mvs.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more MVSs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.mvs.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.mvs.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.mvs.balanceOf(bob)).valueOf(), '98');
    });
});
